...
public class LinkedList<E> {
    ...
    //convertir Array a LinkedList
    public void fromArray(E[] array) {
        this.clear(); // Limpiar la lista actual antes de agregar nuevos elementos
        for (E elemento : array) {
            this.add(elemento); // Suponiendo que "add" es el método para agregar elementos
        }
    }

    public void ordenarQuickSort(Integer type) throws Exception {
        if (this.isEmpty()) {
            throw new Exception("La lista está vacía. No se puede ordenar.");
        }
        E[] array = this.toArray(); // Convierte la lista a un arreglo
        quickSort(array, 0, array.length - 1, type); // Llama al método QuickSort
        this.fromArray(array); // Convierte el arreglo ordenado de nuevo a una lista
    }
    
    public void ordenarMergeSort(Integer type) throws Exception {
        if (this.isEmpty()) {
            throw new Exception("La lista está vacía. No se puede ordenar.");
        }
        E[] array = this.toArray();
        mergeSort(array, 0, array.length - 1, type);
        this.fromArray(array);
    }
    
    public void ordenarShellSort(Integer type) throws Exception {
        if (this.isEmpty()) {
            throw new Exception("La lista está vacía. No se puede ordenar.");
        }
        E[] array = this.toArray();
